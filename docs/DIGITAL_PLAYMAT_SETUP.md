...
**Warbands** — edit `src/data/playmat/warbands.ts`. Included:

- The Gnarlspirit Pack (4 fighters)
- Sons of Velmorn (5)
- Grinkrak's Looncourt (7)
- Xandire's Truthseekers (4)
- Drepur's Wraithcreepers (4 — no images yet, see below)

Each warband may have **3 to 7 fighters** — add or remove entries in its
`fighters` array; `wounds` is the printed Wounds characteristic.

**Decks** — the four fixed Rivals decks live in `src/data/playmat/rivalsDecks.ts`
(generated from the wuclub card database, including full rules text):
...
- `public/warbands/<warbandId>/…webp` — fighter cards + inspired sides +
  warscrolls for Gnarlspirit Pack, Sons of Velmorn, Grinkrak's Looncourt, and
...
card pool, filter by type / Surge / Ploy, view any card full-size (with rules
text), and create / edit / duplicate / delete decks. Decks can also be exported
and imported as a share code.
...
src/data/playmat/rivalsDecks.ts    the four fixed decks (generated)
...
scripts/fetch-wuclub-assets.mjs    re-download card/fighter images
```

> **Tip:** commit the playmat files to git. The first version of this work was
> lost to an uncommitted branch switch — don't let it happen twice.
